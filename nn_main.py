...
def optimize_eta(eta, theta, loss_fn):
    X, Y, val_X, val_Y = get_mnist_data()
    meta_eta, bsz = args.eta, args.bsz
    randperm = np.random.permutation(X.shape[0])
    X, Y = X[randperm], Y[randperm]
    Xs, Ys = batchify(X, Y, bsz)
    Xs = Xs[:4]
    ...
    for i in range(1, args.nepochs+1):
        grad, mb_loss = gvf(eta, theta, Xs, Ys)
        eta = eta - meta_eta * grad
        eta = eta.item()
        print(f'eta at iteration {i}:', eta)
...
if __name__ == "__main__":
    ...
    if args.problem == "6.1":
        ...
        b = np.zeros((mnistH1,))
        theta = [W2, W1, b]
        optimize_eta(eta, theta, utils.eta_xent)
